...
def excel_deudas(request):
  ...
  for deuda in deudas:
    ...
    sheet.write('D%s' % row, deuda.registro_padre.total_venta, money)
...
def excel_ventas(request):
  ...
  sheet.write('J3', u'Descuento_soles', bold)
  ...
  sheet.write('T3', u'Total Detalles', bold)
  ...
  for venta in ventas:
    ...
    sheet.write('J%s' % row, venta.descuento_soles)
    ...
    sheet.write_formula('T%s' % row, '{=H%s*K%s-J%s}' % (row, row, row))
    row += 1
